...
//dbcon();
     ...
     if($c_choice=='1'){
$query = mysqli_query($condb,"select appNo , FirstName, SecondName,first_Choice, J_score, post_uscore from $table where first_Choice = '$depart' and Asession = '$session' and reg_status = '1' and verify_apply = 'TRUE' and app_type ='".safee($condb,$class_ID)."' ")or die(mysqli_errno($condb));
}else{
$query = mysqli_query($condb,"select appNo , FirstName , SecondName ,Second_Choice , J_score , post_uscore from $table where Second_Choice = '$depart' && Asession = '$session' && reg_status = '1' && verify_apply = 'TRUE' and app_type ='".safee($condb,$class_ID)."' ")or die(mysqli_errno($condb));
}
